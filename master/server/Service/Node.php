...
class Node extends \Sky\Service implements \Sky\Service\IService
{
    ...
    public function delNode($server,$fd, $from_id)
    {
        ...
        if (isset($this->sky->nodes[$fd]) and !empty($this->sky->nodes[$fd]))
        {
            ...
            if (table('node')->exists(array('ip' => $this->sky->nodes[$fd]['host'])))
            {
                ...
                $insert['ip'] = $this->sky->nodes[$fd]['host'];
                table('node')->set($this->sky->nodes[$fd]['host'],array('last_time'=>date('Y-m-d H:i:s')),'ip');
            }
            if (!empty($this->sky->ctl))
            {
                foreach ($this->sky->ctl as $f => $info)
    ...
    public function addName($server, $fd, $from_id,$params)
    {
        ...
        if (isset($this->sky->nodes[$fd]) and !empty($this->sky->nodes[$fd]))
        {
            ...
            //node节点上报名字 存储节点到mysql
            if (!table('node')->exists(array('ip' => $this->sky->nodes[$fd]['host'])))
            {
                $insert['alias'] = $this->sky->nodes[$fd]['name'];
                $insert['ip'] = $this->sky->nodes[$fd]['host'];
                table('node')->put($insert);
            }
        }
        if (!empty($this->sky->ctl))
        {
